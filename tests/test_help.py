...
def _bare_screen() -> HelpScreen:
    """Create a HelpScreen without Textual's widget init (no app needed)."""
...
def _mock_app(screen: HelpScreen):
    """Temporarily patch HelpScreen.app as a property returning a mock app."""
    mock_app = MagicMock()
...
class TestPixelGraphicsAvailable:
    ...
    def test_returns_false_when_auto_is_halfcell(self, monkeypatch):
        """Returns False when auto selected a non-pixel renderer (halfcell, unicode)."""
        import src.tina.main as main_mod
